...
extension XCTestCase {
    ...
    func assertDeallocation(of objects: () -> (UIViewController, [AnyObject])) {
        ...
        wait(
            ...
            timeout: 3.0,
            description: "The view controller should be deallocated."
        )
        wait(
            for: weakReferenceDependencies.filter { $0.value != nil }.isEmpty,
            timeout: 3.0,
            description: "Dependencies of the controller should be deallocated."
        )
    }
    ...
    func wait(for condition: @autoclosure @escaping () -> Bool,
        ...
        XCTAssertTrue(
            value,
            "Timed out waiting for condition to be true: \"\(description)\"",
            file: file,
            line: line
        )
    }
